...
- 本のカバー画像（表紙）は、本のディレクトリ直下に `cover.png`（または `cover.jpeg`）を置く。推奨サイズは 500×700px。Claude Code では `.claude/skills/book-cover-designer/` でミニマルなカバーを生成できる。
...
- 自動で強制（エラー）する主な制約は次のとおり。
  ...
  - `no-doubled-joshi`（同一助詞の連続）を検出する。`severity` を `error` とし、commit と CI を止める。
- である調・ですます調は混在のみを検査し、特定の調子は強制しない。管理文書は である調 で書く。記事の調子は記事ごとに選ぶ。
- 本リポジトリは textlint の warning を使わず、全ルールを `error` で運用する（textlint 自体は `severity: "warning"` も扱えるが、本リポジトリでは採用しない）。pre-commit（lefthook）と CI（`.github/workflows/lint.yml`）はともに終了コードで判定するため、検出はすべて commit と CI を止める。
