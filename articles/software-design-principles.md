...
設計原則は、共通の問題への異なる回答である。問題とは「変更に強いコードの普遍則をどう言語化し、いつ破るか」である。原則は教条ではない。効く場面があり、衝突する相手があり、破るべき限界がある。原則を機械的に適用すると、原則が解こうとした痛みより大きな痛みを生む。回答は、次の軸で分岐する。
...
- 上位モジュールと下位モジュールは、いずれも抽象に依存する。具体に依存しない。
...
- 依存性逆転の原則を効かせる目安は、詳細が差し替わる可能性の高い境界（永続化・外部サービス・入出力）である。
...
KISS（Keep It Simple, Stupid）は、設計を可能な限り単純に保つことを促す。複雑さは必要になった時点で導入し、最初から作り込まない。標語は航空機設計者 Kelly Johnson（Lockheed）に帰されるが、出典は確実でなく、俗にそう語られる[^kiss-origin]。
...
- 関連: 過度な複雑さを避ける思想は、Unix 哲学[^unix-philosophy]や Antoine de Saint-Exupéry の「完璧とは、足すものがなくなったときではなく、引くものがなくなったときに達する」とする言葉に通じる。
...
多通貨と丸めモードは、要求が現れた時点で足す。今は使わない柔軟性を持ち込まない。
...
- 凝集度と結合度は、複数の SOLID 原則（SRP・ISP・DIP）とデメテルの法則の源流である。後発の原則の多くは、高凝集・疎結合という古い尺度を別の角度から具体化したものである。
