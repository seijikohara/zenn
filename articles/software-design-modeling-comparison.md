...
- 汎用のビジネスアプリケーションでは、ECS の利点（大量エンティティの一括処理、データ局所性）が活きる場面が乏しい。
...
ルールは `Order` に属する。与信検査も合計計算も `Order` のメソッドに集約され、他のユースケースから同じメソッドを呼べる。集約は整合性の境界を定め、境界内の不変条件をまとめて守る。
...
- 提唱: David Harel が Statecharts を 1987 年の論文で示した。論文名は "Statecharts: A Visual Formalism for Complex Systems"、掲載誌は Science of Computer Programming である[^sm-harel]。
...
導入で示した 4 軸（データと振る舞い・状態とイベント・型による制約・コードと DB の主従）が、関係の背骨を成す。各思想は同じ軸の両端で対立し、軸の上で近い思想は系譜や合成でつながる。5 族で束ねた地図に系譜・対立・合成の線を引くと、思想の選択は孤立した 10 択ではなく、軸の上での位置取りと組み合わせの判断になる。
...
- 状態と履歴の要求が保存方法を決める。履歴に価値があればイベントソーシング、現在状態の遷移規則が中心なら状態機械が向く。
