...
本章では、Google AdMob[^admob] を使って、アプリにバナー広告を表示する最小の手順を扱います。

- アプリは無料で配り、広告で収益を得る。
- 広告の収益は広告ネットワークから受け取るため、Google Play の決済プロファイルは不要。
...
Google Mobile Ads SDK を追加します。執筆時点の最新版は 25.3.0 です。SDK は次の要件があります[^admob-relnotes]。
...
AndroidManifest に AdMob のアプリ ID を設定します。次はテスト用のサンプルアプリ ID です。本番では、自分の AdMob アプリ ID に置き換えます。

:::message alert
アプリ ID の設定が無いと、アプリが起動時にクラッシュします。
:::
...
アプリ ID と広告ユニット ID は、区切り文字が異なります。
