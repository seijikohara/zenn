...
初回起動時に Setup Wizard（セットアップウィザード）が開きます。Setup Wizard は、開発に必要な Android SDK[^sdk] のコンポーネントをダウンロードし、開発環境を構成します。画面の指示に従い、推奨される SDK パッケージをインストールします。

:::message
ダウンロード容量は数 GB に及ぶ場合があり、回線速度によっては完了まで時間がかかります。
:::
...
入力した情報の一部は Google Play 上で公開されます。公開される情報はアカウント種別で異なるため、種別を選ぶ前に次節を確認してください。
...
登録の過程で本人確認を求められます。必要な情報はアカウント種別で異なります。

- 個人アカウント: 政府発行の本人確認書類、法的氏名と住所、確認済みのメールアドレスが必要です。
- 組織アカウント: 追加で D-U-N-S 番号・組織の公式書類・電話番号・Web サイトの確認が必要です[^verify]。
