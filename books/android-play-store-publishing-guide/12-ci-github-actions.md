...
自動配信の設定は、2 つの管理画面を使います。Google Cloud Console でサービスアカウントを作成し、Play Console で、作成したサービスアカウントへ配信の権限を与えます。発行した JSON 鍵を GitHub Secrets に登録すると、CI から配信できます。
...
`status` を `completed` にすると即時に反映されます。段階的に公開する場合は `status: inProgress` と `userFraction` を併用します。
