...
本章では、ビルド・署名・公開でよくある詰まりと対処をまとめます。症状から原因と対処を引けるよう、項目別に整理します。

## ビルドの問題

ビルドで発生する代表的な症状と対処は次のとおりです。

| 症状 | 原因 | 対処 |
| --- | --- | --- |
| `kotlin-android` プラグインでビルドが失敗する | AGP 9.0 以降は Kotlin の組み込みサポートが既定で有効 | `org.jetbrains.kotlin.android` プラグインを削除する |
| Compose コンパイラと Kotlin のバージョンが一致しない | Compose コンパイラプラグインのバージョン不一致 | Compose コンパイラプラグインのバージョンを Kotlin にそろえる |
| JDK が新しすぎて Gradle が動かない | Gradle が対応するより新しい JDK で起動に失敗 | 対応する JDK（本書では 17）を使う |
| SDK プラットフォームが見つからない | `compileSdk` の API レベルのプラットフォームが無い | SDK Manager または `sdkmanager` で対象の API レベルを導入する |
...
アップロード鍵を紛失した場合は、新しい鍵を作成し、Play Console からアップロード鍵のリセットを申請します[^reset]。

:::message
リセットはアプリ署名鍵に影響しないため、同じアプリの更新を継続できます。
:::
