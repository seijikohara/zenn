...
本章は第 3 章で mise を導入・有効化した状態を前提とします。mise 自体のインストールとシェル有効化は第 3 章を参照してください。本章は設定ファイルの中身と構文を扱い、ツールを入れる・選ぶといったコマンド操作（`mise install` / `mise use` / `mise ls`）は第 5 章で扱います。環境変数（env）は第 13 章、タスク（tasks）は第 14 章で扱います。
...
mise.toml には `[tools]` 以外に、環境変数を定義する `[env]` セクションと、タスクを定義する `[tasks]` セクションがあります[^config]。`[env]` の詳細は第 13 章、`[tasks]` の詳細は第 14 章で扱います。本章は `[tools]` セクションのバージョン指定に絞って説明します。

mise.toml はプロジェクトのルートに置き、バージョン管理システムにコミットします。チームの全員が同じバージョンのツールを使えます。

## 設定ファイルの種類と探索順

mise が読み込む設定ファイルは、適用範囲によって 2 種類に分かれます。プロジェクト単位の設定ファイルと、ユーザ全体に適用するグローバル設定ファイルです。さらに、asdf[^asdf] と互換の `.tool-versions` も読み込みます。
