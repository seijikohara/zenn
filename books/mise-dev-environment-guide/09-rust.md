...
[^rust-toolchain]: mise が idiomatic version file として読み込む `rust-toolchain.toml`。読み込みは `mise.jdx.dev/lang/rust.html` には明記されないが、mise 本体の Rust プラグイン実装が `rust-toolchain.toml` を対象とする。mise 公式リポジトリ「src/plugins/core/rust.rs」<https://github.com/jdx/mise/blob/main/src/plugins/core/rust.rs>
