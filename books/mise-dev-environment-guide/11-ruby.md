...
`ruby` のバージョン指定には、第 4 章で説明した構文を使います。完全指定・部分指定に加えて、`latest` のエイリアスを利用できます[^ruby]。
...
部分指定は、先頭の一部だけを書きます。`ruby = "3.4"` は、3.4 系の最新バージョン（執筆時点では 3.4.9）に解決されます。`latest` は最新の安定版（執筆時点では 4.0.5）に解決します[^ruby-downloads]。
...
メジャーバージョンだけを書くと、そのメジャー系列の最新版に解決します。執筆時点では `ruby = "3"` は 3.4.9 に、`ruby = "4"` は 4.0.5 に解決します。
...
`xcode-select --install` は、コンパイラを含む Command Line Tools を導入します。`openssl@3` は TLS 通信に、`readline` は対話シェル（irb）の行編集に、`libyaml` は YAML を扱う標準ライブラリ（psych）に、`gmp` は多倍長整数の演算に使われます[^ruby-build-env]。
