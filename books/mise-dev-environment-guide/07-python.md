...
本章は第 3 章で mise を導入・有効化し、第 5 章でバージョン操作を理解した状態を前提とします。mise 自体のインストールは第 3 章、`mise.toml` の構文は第 4 章、`mise install` / `mise use` / `mise ls` などの操作は第 5 章で扱いました。本章はコマンドの基本操作を再説明せず、Python 固有の事情に集中します。pyenv からの移行手順は第 17 章で扱います。
...
`python` のバージョン指定には、第 4 章で説明した構文を使います。完全指定・部分指定に加えて、`latest` のエイリアスを利用できます[^python]。
...
`path` は venv のパスです。プロジェクトのルートからの相対パスで指定します。`create` を `true` にすると、指定したパスに venv が無いときに mise が venv を作成します[^python]。`create` を省略した場合、venv はあらかじめ手動で作成しておく必要があります[^python]。
