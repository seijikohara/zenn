...
2 つの方法の違いは、`mise` 実行ファイルの配置先と、PATH への登録方法です。Homebrew は `mise` を PATH に登録済みのディレクトリへ配置します。スクリプトは `~/.local/bin` へ配置し、シェルの有効化を通じて PATH に追加します。後述するシェル有効化の記述が、導入方法によって変わります。
...
設定ファイルへの追記は、新しいシェルの起動時に反映されます。シェルを再起動するか、設定ファイルを再読み込みします。再読み込みには、zsh では `source ~/.zshrc`、bash では `source ~/.bashrc` を実行します。
