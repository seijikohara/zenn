...
本章は第 3 章で mise を導入・有効化し、第 4 章で mise.toml の基本を理解した状態を前提とします。本章の例には Node.js を用いますが、操作の体系はすべてのツールで共通です。関連する内容は次の章で扱います。

- mise 自体のインストールとシェル有効化: 第 3 章
- mise.toml の設定構文の詳細: 第 4 章
- 環境変数（env）: 第 13 章
- タスク（tasks）: 第 14 章
- Node.js・Python など言語ごとの個別事情: 第 6 章以降
...
`mise install` は `~/.local/share/mise/installs/<ツール>/<バージョン>` にバージョンを配置します[^install]。

:::message
インストールだけでは PATH にツールが入らず、コマンドとして呼び出せません。インストールと有効化を 1 つのコマンドでまとめて行うには、後述の `mise use` を使います。
:::
...
設定したバージョンが正しく適用されているかを、次の 3 つのコマンドで確認します。

| コマンド | 確認できる内容 |
| --- | --- |
| `mise ls` | ツールのバージョン一覧 |
| `mise current` | 現在有効なバージョン番号 |
| `mise which` | 実行ファイルの絶対パス |
