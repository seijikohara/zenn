...
本章は、nvm・pyenv・rbenv などの言語別ツールを使用中で mise へ移行したい開発者を主な対象とします。第 3 章で mise を導入・有効化し、第 5 章でバージョン操作を理解した状態を前提とします。本章は移行の手順に集中し、各ツールの個別事情は再説明しません。関連する内容は次の章で扱いました。

- mise 自体のインストールとシェル有効化: 第 3 章
- `mise.toml` の構文と `.tool-versions` の扱い: 第 4 章
- Node.js のバージョンファイル: 第 6 章
- Python のバージョンファイル: 第 7 章
- Ruby のバージョンファイル: 第 11 章
...
2 つの方法は、プロジェクトの状況に応じて使い分けます。

- 新規のプロジェクト: `mise.toml` を使い、`mise.toml` の機能（`[env]`・`[tasks]`）を活用します。
- 既存のバージョンファイルを持つプロジェクト: 移行期間中は既存ファイルの読み込みが互換性の維持に向きます。
- 移行が済んだプロジェクト: `mise.toml` へ集約すると設定を一元化できます。
