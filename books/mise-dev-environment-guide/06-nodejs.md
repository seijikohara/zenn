...
`node` のバージョン指定には、第 4 章で説明した構文を使います。完全指定・部分指定に加えて、`lts` と `latest` のエイリアスを利用できます[^node]。`lts` は最新の長期サポート版（Long Term Support、LTS）に解決します。各指定形式と解決先を次の表にまとめます。

| 指定形式 | 例 | 解決先 |
| --- | --- | --- |
| 完全指定 | `node = "22.22.3"` | 22.22.3 に固定 |
| 部分指定 | `node = "22"` | 22 系の最新バージョン（執筆時点では 22.22.3） |
| `lts` | `node = "lts"` | 最新の LTS（執筆時点では Node.js 24 系の最新版） |
| `latest` | `node = "latest"` | 最新の安定版 |
| `lts/<コードネーム>` | `node = "lts/jod"` | 指定した LTS 系列の最新版 |

完全指定は、バージョンを 1 つに固定します。
...
部分指定は、先頭の一部だけを書きます。`node = "22"` は、22 系の最新バージョンに解決されます。

`lts` と `latest` は、エイリアスとして書きます[^node]。
...
Node.js のパッケージマネージャには、npm・pnpm・yarn があります。mise での扱いは、ツールごとに異なります。3 つの違いを次の表にまとめます。

| ツール | Node.js への同梱 | mise での宣言 |
| --- | --- | --- |
| npm | 同梱される | 通常は不要（`node` に従う） |
| pnpm | 同梱されない | `pnpm` で宣言 |
| yarn | 同梱されない | `yarn` で宣言 |
