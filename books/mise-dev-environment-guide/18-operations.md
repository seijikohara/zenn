...
mise 本体の更新方法は、第 3 章で選んだ導入方法によって変わります。Homebrew で導入した場合は Homebrew で更新し、公式インストールスクリプトで導入した場合は `mise self-update` で更新します。先に導入方法を確認し、対応する更新方法を選びます。
...
`/opt/homebrew` 以下にある場合は、Homebrew で導入した mise です。`~/.local/bin/mise` にある場合は、公式インストールスクリプトで導入した mise です。Homebrew の配置先は CPU アーキテクチャによって変わり、Apple シリコンでは `/opt/homebrew`、Intel では `/usr/local` です。第 3 章で扱った 2 通りの導入方法に対応します。
...
`mise self-update` は、更新前に確認を求めます。確認を省略するには `-y`（`--yes`）を付けます[^self-update]。特定のバージョンを指定して更新するには、バージョンを引数に渡します[^self-update]。`mise self-update` は、既定でインストール済みのプラグインも併せて更新します[^self-update]。プラグインの更新を無効にするには `--no-plugins` を付けます[^self-update]。
...
`mise settings set` は、グローバル設定ファイルにキーを書き込みます[^settings]。プロジェクト単位で設定する場合は `--local`（`-l`）を付けます。`--local` は、現在のディレクトリの `mise.toml` の `[settings]` に書き込みます[^settings]。設定を削除するには `mise settings unset <キー>` を実行します[^settings]。複数の値を持つ設定にキーを追加するには、第 17 章で扱った `mise settings add` を使います。
...
プラグインを更新するには、`mise plugins update` を実行します[^plugins]。`mise plugins update` は、導入済みのプラグインを最新へ更新します[^plugins]。引数なしで実行すると、すべてのプラグインを対象にします[^plugins]。特定のプラグインだけを更新するには、プラグイン名を引数に渡します[^plugins]。
